...
class PatchMerging(nn.Module):
    r""" 1D Patch Merging layer (token downsampling).

    Merges pairs of adjacent tokens by concatenation + linear projection,
    halving the token length (L -> L/2) and typically doubling channels (C -> 2C).

    Args:
        dim (int): Number of input channels.
        out_dim (int | None): Output channels after reduction. Default: 2*dim.
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
    """
    ...
    def forward(self, x):
        """
        Args:
            x: (B, L, C), L must be even.

        Returns:
            (B, L/2, out_dim)
        """
        B, L, C = x.shape
        assert L % 2 == 0, f'input length ({L}) must be even for 1D patch merging'

        x0 = x[:, 0::2, :]  # even-indexed tokens, (B, L/2, C)
        x1 = x[:, 1::2, :]  # odd-indexed tokens,  (B, L/2, C)
        x = torch.cat([x0, x1], dim=-1)  # (B, L/2, 2C)

        x = self.norm(x)
        x = self.reduction(x)  # (B, L/2, out_dim)
        return x

    def extra_repr(self) -> str:
        return f'dim={self.dim}, out_dim={self.out_dim}'

    def flops(self, L):
        flops = 0
        # norm
        flops += (2 * self.dim) * (L // 2)
        # reduction: Linear(2*dim -> out_dim)
        flops += (L // 2) * (2 * self.dim) * self.out_dim
        return flops
...
class SwinTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.linear_proj = nn.Linear(1, self.dim) # lift dim
        self.mlp_classifier = nn.Linear(dim, self.num_classes) # classifier head
    ...
    def forward(self, x, return_feat=False):
        # lift dim
        x_perm = x.permute(0, 2, 1)  # now shape [BS, L, 1]
        x = self.linear_proj(x_perm)
        #
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x)
        # MLP classifier head: pool + classify
        out_pool = x.mean(dim=1)      # (BS, 128) pooled features
        logits = self.mlp_classifier(out_pool)   # (BS, num_classes)
